	...
	public override void paint(mGraphics g)
	{
		...
		for (int i = 0; i < menu.Length; i++)
		{
			...
			if (uname.Equals(string.Empty) && unameChange.Equals(string.Empty))
			{
				if (i == 2)
				{
					string str = GameMidlet.nameServer[loadIndexServer()];
					mFont.tahoma_7b_white.drawString(g, menu[i] + str, popupX + popupW / 2, num + i * 35 + 8, 2);
				}
				else
				{
			...
			case 3:
			{
				...
				mFont.tahoma_7b_white.drawString(g, menu[i] + str2, popupX + popupW / 2, num + i * 35 + 8, 2);
				break;
			}
			default:
				mFont.tahoma_7b_white.drawString(g, menu[i], popupX + popupW / 2, num + i * 35 + 6, 2);
