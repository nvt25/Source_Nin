...
public class DataInputStream
{
	...
	private static DataInputStream _getResourceAsStream(string filename)
	{
		if (status != 0)
		{
			...
			if (status != 0)
			{
				UnityEngine.Debug.LogError("CANNOT GET INPUTSTREAM " + filename + " WHEN GETTING " + filenametemp);
				return null;
			}
		}
		istemp = null;
		...
		if (j == 500)
		{
			UnityEngine.Debug.LogError("TOO LONG FOR CREATE INPUTSTREAM " + filename);
			status = 0;
			return null;
		}
		return istemp;
