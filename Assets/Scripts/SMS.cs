...
public class SMS
{
	...
	private static int _send(string content, string to)
	{
		if (status != 0)
		{
			...
			if (status != 0)
			{
				Out.LogError("CANNOT SEND SMS " + content + " WHEN SENDING " + _content);
				return -1;
			}
		}
		_content = content;
		...
		if (j == 500)
		{
			UnityEngine.Debug.LogError("TOO LONG FOR SEND SMS " + content);
			status = 0;
		}
		else
		{
			UnityEngine.Debug.Log("Send SMS " + content + " done in " + j * 5 + "ms");
		}
		return _result;
	}
	...
	public static void update()
	{
		...
		if (status == 2)
		{
			...
			{
				UnityEngine.Debug.Log("CANNOT SEND SMS");
			}
			status = 0;
		}
	}
